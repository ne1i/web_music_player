export default function NotFoundPage(props: { request: Request }) {
  const { request } = props;
  return (
    <div className="container mx-auto flex flex-col gap-4 p-4">
      ...
      <a
        className="text-blue-500 underline font-bold hover:text-blue-600 visited:text-purple-600"
        href="/"
      >
        Go to home
      </a>
      ...
      <details className="border-2 border-red-600 border-dashed p-2 text-red-600 bg-red-50">
        <summary className="">
          <h2 className="inline-block mr-2 font-bold">
        ...
        <div className="space-y-4 mt-2">
          <table className="table-auto w-full text-xs text-red-600 border-collapse border border-red-800 p-2">
            ...
            <tbody>
              {Array.from(Object.entries(request.headers)).map(
                ([key, value]) => (
                  <tr key={key}>
          ...
          <code className="overflow-x-auto">
            ...
            <pre className="text-xs">
              Content-Type:{" "}
              {JSON.stringify(request.headers.get("content-type"), null, 2)}
            </pre>
            <pre className="text-xs">
              Content-Length:{" "}
              {JSON.stringify(request.headers.get("content-length"), null, 2)}
            </pre>
            <pre className="text-xs">
              Accept: {JSON.stringify(request.headers.get("accept"), null, 2)}
            </pre>
            <pre className="text-xs">
              Accept-Encoding:{" "}
              {JSON.stringify(request.headers.get("accept-encoding"), null, 2)}
            </pre>
          </code>
        </div>
      </details>
